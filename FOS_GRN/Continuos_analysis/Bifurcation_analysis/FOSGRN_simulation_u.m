function [dxi]=FOSGRN_simulation_u(t,x,tu,u1)
%% Initial parameters
dxi=zeros(13,1); 
w_x=zeros(13,1);
...
w_x(2)=((1-AG)*(1-TFL1*(1-LFY*FT)))+u1-(1-AG)*(1-TFL1*(1-LFY*FT))*u1;
...
% Continuos equation to gene AP3 and controller u2
% f_w_x4=(LFY*UFO)+(PI*SEP*AP3*(AG+AP1-AG*AP1))-(LFY*UFO)*(PI*SEP*AP3*(AG+AP1-AG*AP1));
% w_x(4)=f_w_x4+u2-f_w_x4*u2;
...
w_x(12)=UFO; %Sin control
%w_x(12)=UFO*u1;

% Continuos equation to gene WUS
w_x(13)=WUS*(1-AG*SEP); %Sin control
%w_x(13)=(WUS*(1-AG*SEP))*u1;
